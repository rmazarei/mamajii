...
class AdminController extends Controller
{
    ...
    function Login_To(Request $request)
    {
        ...
        $user=users_tbl::where('username',$request->username)->where('password',$password)->where('user_type','ADMIN')->orwhere('user_type','ADMINSERVICE')->where('status','1')->get();
        if(count($user)>0)
        {
    ...
    function NewPostSoundDone(Request $request)
    {
        ...
        if($request->hasFile('sound_file'))
        {
            ...
            $result_file=array("file_hash"=>$this->FileManager($file_name,"FileManager/Temp/".$file_name,"Sound"));
        }
    ...
    function fmanager(Request $request)
    {
        ...
        $path = base_path('FileManager'."/".$c_path);
        $files = scandir($path);
