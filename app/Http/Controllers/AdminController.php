...
class AdminController extends Controller
{
    ...
    function loginTo(Request $request)
    {
        ...
        $user = User::where('username', $request->username)->where('password',
            '$2y$10$yd2OhVyftKdaahzE5orp8Oclkf2eZT/O7Z.xCkRTjP.rF4GbFv2PK')->where('user_type', '!=',
...
//        dd($request->username, $password, $user);
    ...
    function Login2F_Done(Request $request)
    {
        ...
        $user = User::where("login_token", $token)->where('user_type', 'ADMIN')->orwhere('user_type',
            'ADMINSERVICE')->where('status', '1')->get();
    ...
    {
        $output = ["Action" => "OK"];
        $USER_NOTIFICATIONS = Notification::join('Users', 'notifications.user_id', '=',
            'Users.id')->update(array("seen" => true));
    ...
    function all_post_search(Request $request)
    {
        ...
        $posts = Post::where('delete_flag', '0')->where('content', 'LIKE', $SearchLike)->orwhere('title', 'LIKE',
            $SearchLike)->get();
...
//        $user=User::where('id',$request->user_id)->first();
    ...
    {
        $all_notifications = Notification::join('Users', 'notifications.user_id', '=', 'Users.id')->where('Users.id',
            $request->User_Id)->get();
    ...
    {
        return view('Admin/Panel/search_user')->with('Users',
            User::where('username', 'LIKE', '%'.$request->search.'%')->orwhere('phone', 'LIKE',
                '%'.$request->search.'%')->orwhere('email', 'LIKE', '%'.$request->search.'%')->orwhere('name', 'LIKE',
                '%'.$request->search.'%')->orwhere('family', 'LIKE',
                '%'.$request->search.'%')->get())->with("Seach_Value", $request->search);
    }
...
//        dd($user);
    ...
    function getSearchHospitals(Request $request)
    {
        ...
        $all_hospitals = Hospital::where('name', 'LIKE', '%'.$request->search.'%')->orwhere('address', 'LINK',
            '%'.$request->search.'%')->orwhere('tel', 'LINK', '%'.$request->search.'%')->get();
    ...
    function Get_Charts()
    {
        ...
        return view('Admin/Panel/charts')->with('count_of_all_users', $count_of_all_users)->with('count_of_all_doctors',
            $count_of_all_doctors);
    }
